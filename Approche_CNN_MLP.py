...
print(f"Train : {len(training_set)} | Val : {len(validation_set)} | Test : {len(test_set)}")
...
os.makedirs("runs/resnet50", exist_ok=True)
...
for epoch in range(N_EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model_resnet.state_dict(), "runs/resnet50/resnet50_best.pth")
...
model_resnet.load_state_dict(torch.load("runs/resnet50/resnet50_best.pth", map_location=device))
model_resnet.eval()
