...
class FaceOcclusionDataset(torch.utils.data.Dataset):
    def __init__(self, df, image_dir, has_label=True, augment=False):
...
training_set = FaceOcclusionDataset(
    df_train,
    image_dir,
    has_label=True,
    augment=True
...
validation_set = FaceOcclusionDataset(
    df_val,
    image_dir,
    has_label=True,
    augment=False
...
internal_test_set = FaceOcclusionDataset(
    df_internal_test,
    image_dir,
    has_label=True,
    augment=False
...
os.makedirs("runs/resnet50_aug", exist_ok=True)
...
for epoch in range(N_EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model_resnet.state_dict(), "runs/resnet50_aug/resnet50_aug_best.pth")
        print("✓ meilleur modèle sauvegardé")

    scheduler.step()

    print(f"LR actuel : {scheduler.get_last_lr()[0]:.8f}")
# In[ ]:
...
model_resnet.load_state_dict(torch.load("runs/resnet50_aug/resnet50_aug_best.pth", map_location=device))
model_resnet.eval()
...
model_resnet.load_state_dict(
    torch.load("runs/resnet50_aug/resnet50_aug_best.pth", map_location=device)
)
model_resnet.eval()
